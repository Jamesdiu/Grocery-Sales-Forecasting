...
set.seed(1111)
train <- sample_frac(as.tibble(fread("train.csv")),0.2)
test <- fread("test.csv")
...
holidays %>% filter(transferred != FALSE)
```

items.csv - Item metadata, including family, class, and perishable.
NOTE: Items marked as perishable have a score weight of 1; otherwise, the weight is 0.
```{r items}
head(items)
```
...
```{r trans}
head(trans)
trans <- trans %>% mutate(date = as.Date(date),
                          day = day(date),
                          month = month(date),
                          year = year(date),
                          wday = wday(date))
...
train.csv - Training data, which includes the target unit_sales by date, store_nbr, and item_nbr and a unique id to label rows.
